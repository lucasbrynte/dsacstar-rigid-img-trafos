...
def interp_bilinear(img, idx_x, idx_y):
    ...
    input:
        img - [batch,C,H,W] tensor from which values are interpolated
    ...
    img_interp  = (idx_x_2 - idx_x).unsqueeze(0).unsqueeze(0)*(idx_y_2 - idx_y).unsqueeze(0).unsqueeze(0)*img[:, :, idx_y_1, idx_x_1]
    img_interp += (idx_x - idx_x_1).unsqueeze(0).unsqueeze(0)*(idx_y_2 - idx_y).unsqueeze(0).unsqueeze(0)*img[:, :, idx_y_1, idx_x_2]
    img_interp += (idx_x_2 - idx_x).unsqueeze(0).unsqueeze(0)*(idx_y - idx_y_1).unsqueeze(0).unsqueeze(0)*img[:, :, idx_y_2, idx_x_1]
    img_interp += (idx_x - idx_x_1).unsqueeze(0).unsqueeze(0)*(idx_y - idx_y_1).unsqueeze(0).unsqueeze(0)*img[:, :, idx_y_2, idx_x_2]
    return img_interp
